...
public class CameraImageAccess : MonoBehaviour
{
   ...
   private Image.PIXEL_FORMAT mPixelFormat =  Vuforia.Image.PIXEL_FORMAT.RGB888;
    ...
    public void Init()
    {
        //Vuforia.VuforiaARController.Instance.UnregisterVuforiaStartedCallback(OnVuforiaStarted);
        //Vuforia.VuforiaARController.Instance.UnregisterOnPauseCallback(OnPause);
        //Vuforia.VuforiaARController.Instance.UnregisterTrackablesUpdatedCallback(OnTrackablesUpdated);
        Start();
        OnVuforiaStarted();
        RegisterFormat();
    }
    ...
    private void OnTrackablesUpdated()
    {
        ...
        {
            if (mAccessCameraImage && !VuforiaOnOff)
            {
                ...
                if (image != null && image.IsValid())
                {
                    ...
                    if (pixels != null && pixels.Length > 0)
                    {
                        ...
                        Texture2D tex = new Texture2D(image.Width, image.Height, TextureFormat.RGB24, false); // RGB24
                        tex.LoadRawTextureData(pixels);
                        tex.Apply();
                        Vector2 screenSize = rawImage.GetComponentInParent<Canvas>().GetComponent<RectTransform>().sizeDelta;
                        rawImage.GetComponent<RectTransform>().sizeDelta = new Vector2(screenSize.y, screenSize.x); // new Vector2(tex.width, tex.height);
